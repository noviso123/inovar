...
def detect_project_type(project_path: Path) -> dict:
    ...
    if (project_path / "package.json").exists():
        add_node_linters(project_path, result["linters"])

    # Node.js project (Frontend)
    if (project_path / "frontend" / "package.json").exists():
        add_node_linters(project_path / "frontend", result["linters"], name_prefix="frontend:")

    # Python project
    if (project_path / "pyproject.toml").exists() or (project_path / "requirements.txt").exists():
        result["type"] = "python"
        result["linters"].append({"name": "ruff", "cmd": ["ruff", "check", "."], "cwd": project_path})
        if (project_path / "mypy.ini").exists() or (project_path / "pyproject.toml").exists():
            result["linters"].append({"name": "mypy", "cmd": ["mypy", "."], "cwd": project_path})

    return result

def add_node_linters(path: Path, linters: list, name_prefix: str = ""):
    try:
        ...
        if "lint" in scripts:
            linters.append({"name": f"{name_prefix}npm lint", "cmd": ["npm", "run", "lint"], "cwd": path})
        elif "eslint" in deps:
            linters.append({"name": f"{name_prefix}eslint", "cmd": ["npx", "eslint", "."], "cwd": path})

        # Check for TypeScript
        if "typescript" in deps or (path / "tsconfig.json").exists():
            linters.append({"name": f"{name_prefix}tsc", "cmd": ["npx", "tsc", "--noEmit"], "cwd": path})
...
def run_linter(linter: dict, cwd: Path) -> dict:
    ...
    try:
        ...
        # Use linter-specific CWD if available, else default
        working_dir = str(linter.get("cwd", cwd))
