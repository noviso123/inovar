...
def detect_test_framework(project_path: Path) -> dict:
    ...
    validation_paths = [project_path, project_path / "frontend"]
    ...
    for path in validation_paths:
        ...
        if package_json.exists():
            ...
            try:
                ...
                if "test" in scripts and "checklist.py" not in test_cmd:
    ...
    # Python project (Fallback if no node tests found)
    if not result["cmd"]:
...
def main():
    ...
    # Run tests
    # Run tests
    result = run_tests(cmd, test_info["cwd"])
