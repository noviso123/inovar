...
export const SystemSettings: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-500">
      <div className="bg-white rounded-[3rem] p-10 border-2 border-slate-50 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {Object.entries(slas).map(([priority, hours]) => (
            <div key={priority} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-2">
                    <input
                        ...
                        className="text-2xl font-black text-slate-800 tracking-tight bg-transparent w-20 outline-none border-b border-transparent focus:border-blue-500"
                    />
                    <span className="font-bold text-slate-400">Horas</span>
                </div>
              </div>
              <button
                onClick={() => handleUpdate(`sla_${priority.toLowerCase()}`, settings[`sla_${priority.toLowerCase()}`])}
                className="px-4 py-2 bg-white text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm hover:bg-blue-600 hover:text-white transition-all"
            >
                Salvar
            </button>
            </div>
      ...
      <div className="bg-white rounded-[3rem] p-10 border-2 border-slate-50 shadow-sm relative overflow-hidden">
        ...
        <h3 className="text-2xl font-black text-slate-800 mb-8 flex items-center gap-4 relative z-10">
        ...
        <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 relative z-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                     ...
                     <div className="flex items-center gap-3">
                        <input
                            ...
                            className="text-4xl font-black text-slate-800 tracking-tight bg-transparent w-28 outline-none border-b-2 border-slate-200 focus:border-emerald-500"
                        />
                ...
                <button
                    ...
                    className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-slate-900/10 hover:bg-emerald-600 transition-all active:scale-95"
                >
                    Salvar Configuração
                </button>
            </div>
      ...
      <div className="bg-slate-900 rounded-[3rem] p-10 text-white relative overflow-hidden">
        ...
        <h3 className="text-2xl font-black mb-6 relative z-10">Políticas de Segurança Real-Time</h3>
