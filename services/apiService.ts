...
class ApiService {
  ...
  // Requests
  async getRequests(filters?: { status?: string; priority?: string; clientId?: string }): Promise<any[]> {
    const params = new URLSearchParams();
    if (filters?.status) params.append('status', filters.status);
    if (filters?.priority) params.append('priority', filters.priority);
    if (filters?.clientId) params.append('clientId', filters.clientId);

    const response = await this.request<{ data: any[] }>(`/requests?${params}`);
    return response.data;
  }

  async createRequest(data: any): Promise<any> {
    const response = await this.request<{ data: any }>('/requests', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return response.data;
  ...
  async updateRequestStatus(id: string, status: string, observation?: string): Promise<any> {
