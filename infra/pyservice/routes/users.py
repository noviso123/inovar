...
from models import User, Prestador, Tecnico
...
@router.get("")
def list_users(company_id: str = "", role: str = "", db: Session = Depends(get_db)):
    q = db.query(User).filter(User.deleted_at.is_(None))
    if company_id:
        q = q.filter(User.company_id == company_id)
    if role:
        q = q.filter(User.role == role)
    return {"data": [user_to_dict(u) for u in q.order_by(User.created_at.desc()).all()]}
