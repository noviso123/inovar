...
export const ClientManager: React.FC<ClientManagerProps> = ({ currentUser }) => {
  ...
  const handleResetPassword = async (clientName: string, id: string) => {
    if (window.confirm(`Deseja resetar a senha de "${clientName}"? A nova senha padrão será: inovar123`)) {
      try {
          await apiService.adminResetPassword(id);
          alert(`Senha de ${clientName} resetada com sucesso para: inovar123`);
      } catch(err) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedClient?.name || !selectedClient?.email) return;

    try {
        if (selectedClient.id) {
            await apiService.updateClient(selectedClient.id, selectedClient);
        } else {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        <button
          onClick={() => { setSelectedClient({ name: '', email: '', phone: '' }); setIsEditing(true); }}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-[70] flex items-center justify-center p-4">
          <form onSubmit={handleSave} className="bg-white rounded-[3.5rem] p-12 w-full max-w-xl shadow-2xl animate-in fade-in zoom-in duration-300 text-left relative">
            <div className="flex justify-between items-center mb-10">
               <h3 className="text-3xl font-black text-slate-800 tracking-tight">{selectedClient?.id ? 'Editar' : 'Novo'} Cliente</h3>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <input required className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-blue-600 focus:bg-white transition-all font-bold"
                  value={selectedClient?.name} onChange={e => setSelectedClient({...selectedClient, name: e.target.value})} />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">E-mail Corporativo</label>
                <input required type="email" className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-blue-600 focus:bg-white transition-all font-bold"
                  value={selectedClient?.email} onChange={e => setSelectedClient({...selectedClient, email: e.target.value})} />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">CNPJ / CPF</label>
                <input required className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none focus:border-blue-600 focus:bg-white transition-all font-bold font-mono"
                  value={selectedClient?.phone} onChange={e => setSelectedClient({...selectedClient, phone: e.target.value})} placeholder="000.000.000-00" />
              </div>

              {selectedClient?.id && isManager && (
                <div className="pt-6 border-t border-slate-100 mt-6">
                  <button
                    type="button"
                    onClick={() => handleResetPassword(selectedClient.name!, selectedClient.id!)}
                  ...
                  <p className="text-[10px] text-center text-slate-400">Senha padrão: inovar123</p>
                </div>
              )}
            </div>

            <div className="flex gap-4 mt-8">
              <button type="submit" className="w-full py-5 bg-slate-900 text-white font-black rounded-2xl shadow-xl shadow-slate-900/20 uppercase tracking-widest text-xs hover:bg-emerald-600 transition-colors">Confirmar Dados</button>
            </div>
          </form>
        </div>
      )}
          ...
          {clients.map(client => (
            <div key={client.id} className={`group relative bg-white p-8 rounded-[2.5rem] border-2 transition-all hover:shadow-2xl hover:-translate-y-1 ${client.active ? 'border-slate-100 hover:border-blue-100' : 'border-rose-50 bg-rose-50/10'}`}>
                ...
                <div className="flex gap-3">
                    <button
                        onClick={() => { setSelectedClient(client); setIsEditing(true); }}
