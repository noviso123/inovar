...
import { User, Equipment, Priority, RequestStatus, ServiceRequest, UserRole } from '../types';
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-[3.5rem] p-8 shadow-2xl border border-slate-50 animate-in zoom-in duration-500 text-left relative">
            ...
            <div className="absolute inset-0 bg-white/95 z-50 rounded-[3.5rem] p-8 flex flex-col animate-in fade-in duration-300">
                ...
                <form onSubmit={handleAddEquipment} className="space-y-4 flex-1 overflow-y-auto">
                    <div>
                        ...
                        <input required className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-100 font-bold" value={newEquip.brand} onChange={e => setNewEquip({...newEquip, brand: e.target.value})} placeholder="Samsung" />
                    </div>
                    <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Modelo</label>
                        <input required className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-100 font-bold" value={newEquip.model} onChange={e => setNewEquip({...newEquip, model: e.target.value})} placeholder="WindFree" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-100 font-bold" value={newEquip.btu} onChange={e => setNewEquip({...newEquip, btu: Number(e.target.value)})}>
                                {[7000,9000,12000,18000,24000,36000,48000,60000].map(v => <option key={v} value={v}>{v}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Local</label>
                            <input required className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-100 font-bold" value={newEquip.location} onChange={e => setNewEquip({...newEquip, location: e.target.value})} placeholder="Sala 1" />
                        </div>
                    </div>
        ...
        <div className="absolute inset-0 bg-white/95 z-50 rounded-[3.5rem] p-8 flex flex-col animate-in fade-in duration-300">
          ...
          <form onSubmit={handleAddClient} className="space-y-4 flex-1 overflow-y-auto">
            ...
            <div>
              ...
              <input className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-100 font-bold" value={newClient.phone} onChange={e => setNewClient({...newClient, phone: e.target.value})} placeholder="(11) 99999-9999" />
            </div>
            <div>
            ...
            <div>
              ...
              <input className="w-full p-4 bg-slate-50 rounded-xl border-2 border-slate-100 font-bold" value={newClient.document} onChange={e => setNewClient({...newClient, document: e.target.value})} placeholder="000.000.000-00" />
            </div>
      ...
      {step === 3 && (
        <div className="space-y-6">
          ...
          <button onClick={handleFinish} className="w-full py-6 bg-slate-900 text-white rounded-[1.5rem] font-black uppercase text-xs tracking-[0.2em] shadow-2xl shadow-slate-900/30 hover:bg-emerald-600 transition-colors">Gerar OS</button>
        </div>
      )}
