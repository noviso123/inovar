...
export const Profile: React.FC<ProfileProps> = ({ user, onUpdateUser, rolePrefix = 'prestador' }) => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setUploading(true);
      try {
        const response = await imageUploadService.uploadFile(file, `avatar-${user.id}`);
        if (response.success) {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        const response = await imageUploadService.uploadFile(file, `logo-${user.companyId || 'company'}`);
        if (response.success) {
          const newUrl = response.data.url;
      ...
      {user.role === UserRole.ADMIN && (
        <div className="mt-6 px-0 relative z-20">
          <div className="bg-white rounded-[2rem] p-6 shadow-xl border border-slate-100">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Logo da Empresa</h3>
            <div className="flex items-center gap-6">
              <div className="relative group w-24 h-24 bg-slate-50 rounded-2xl border-2 border-slate-100 flex items-center justify-center overflow-hidden">
                {logoUploading ? (
                ...
                <label className="absolute inset-0 bg-emerald-600/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                  <span className="text-white font-black text-[9px] uppercase tracking-widest">Alterar Logo</span>
              ...
              <div className="flex-1">
                <p className="text-[10px] text-slate-500 font-bold leading-relaxed lowercase">
                  esta logo será utilizada em todos os documentos gerados pelo sistema (orçamentos, ordens de serviço e notas fiscais). recomendável formato png com fundo transparente.
                </p>
              </div>
            </div>
          </div>
