...
import { ServiceRequest, RequestStatus, User, UserRole, OrcamentoItem, OrcamentoSugestao, NotaFiscal, Attachment } from '../types';
    ...
    if (!schedulingDate || !schedulingTime) {
      ...
      try {
        await apiService.updateRequestStatus(
          ...
          `Agendado para: ${new Date(dateTime).toLocaleString()}`,
          undefined,
          undefined,
            ...
            <section className="space-y-6">
                <div>
                     <h4 className="font-black text-slate-800 text-sm mb-3 flex justify-between items-center">
                        ...
                        <button onClick={handleAddChecklist} className="text-[10px] text-blue-600 font-bold uppercase hover:bg-blue-50 px-2 py-1 rounded-lg">+ Novo Item</button>
                     </h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     ...
                     <div>
                        <h4 className="font-black text-slate-800 text-sm mb-2">Próxima Preventiva</h4>
            ...
            <div className="bg-blue-50/50 p-6 rounded-[2rem] border border-blue-100/50">
              ...
              <div className="space-y-4">
                      ...
                      <button
                        ...
                        onClick={() => {
                            if(confirm('Remover agendamento atual?')) {
                                apiService.updateRequestStatus(request.id, request.status, undefined, undefined, undefined, "NULL").then(reloadRequest);
                            }
                        }}
                      >
                          <Trash2 className="w-5 h-5" />
                ...
                <div className="mt-8 pt-6 border-t border-blue-200/50">
                  ...
                  <div className="space-y-3">
                    {request.history
                      ?.filter((h: any) => h.details?.includes('Agendado para') || h.action?.includes('AGENDADA'))
                      ...
                      .map((h: any, i: number) => (
                        <div key={i} className="bg-white p-3 rounded-xl border border-blue-100 flex items-center gap-3 shadow-sm">
                           ...
                           <div>
                              ...
                              <p className="text-[10px] text-slate-400 capitalize font-medium">{new Date(h.createdAt).toLocaleString()} • {h.userName}</p>
                           </div>
                        </div>
                    ))}
                    {(!request.history || !request.history.some((h: any) => h.details?.includes('Agendado para') || h.action?.includes('AGENDADA'))) && (
