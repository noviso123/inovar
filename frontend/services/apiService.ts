...
class ApiService {
  ...
  async updateRequestStatus(id: string, status: string, observation?: string, materialsUsed?: string, nextMaintenanceAt?: string, scheduledAt?: string): Promise<any> {
    const response = await this.request<{ data: any }>(`/requests/${id}/status`, {
      method: 'PATCH',
      body: JSON.stringify({ status, observation, materialsUsed, nextMaintenanceAt, scheduledAt }),
    });
    return response.data;
  }
